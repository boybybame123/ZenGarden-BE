...
namespace ZenGarden.Tests.Services
{
    public class ItemServiceTests
    {
        ...
        public async Task GetAllItemsAsync_ShouldReturnAllItems()
        {
            ...
            {
                new Item { ItemId = 1, Name = "Item 1", Type = ItemType.Background, Rarity = "Common" },
                new Item { ItemId = 2, Name = "Item 2", Type = ItemType.Background, Rarity = "Rare" }
            };

            var itemDtos = new List<ItemDto>
            {
                new ItemDto { ItemId = 1, Name = "Item 1", Type = ItemType.Background, Rarity = "Common" },
                new ItemDto { ItemId = 2, Name = "Item 2", Type = ItemType.Avatar, Rarity = "Rare" }
            };
        ...
        public async Task CreateItemAsync_WhenCommitFails_ShouldThrowInvalidOperationException()
        {
            ...
            // Act & Assert
            var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => _itemService.CreateItemAsync(createItemDto));
        ...
        public async Task UpdateItemAsync_WithNonExistentItem_ShouldThrowKeyNotFoundException()
        {
            ...
            // Act & Assert
            var exception = await Assert.ThrowsAsync<KeyNotFoundException>(() => _itemService.UpdateItemAsync(updateItemDto));
            Assert.Equal($"Item with ID {itemId} not found.", exception.Message);
        }
        ...
        public async Task UpdateItemAsync_WhenCommitFails_ShouldThrowInvalidOperationException()
        {
            ...
            // Act & Assert
            var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => _itemService.UpdateItemAsync(updateItemDto));
